...
\code{initialize-instance} (thus avoiding undesirable side-effects) and
which \code{typep} and \code{eql} view as traditional
...
\code{ensure-finalized} from the (portable) \code{closer-mop} package \cite{bib:costanza.c2mop}.
...
require to hook inside the implementation. Since conditions are classes, they
are supported automatically. The Common Lisp standard \cite{bib:ansi.94.cl}
states that ``\code{defstruct} without a \code{:type} option defines a class
with the structure name as its name'', hence in that case no additional work is
required. The standard also states that ``Specifying this option [...] prevents
the structure name from becoming a valid type specifier recognizable by
\code{typep}.'' Thus, \code{subtypep} is not concerned by these types of
structures.
