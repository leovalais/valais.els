...
\end{enumerate}
For example, \code{(member a 1 2 :b)} is converted to
\code{(or (member a :b) (bit 1 1) (integer 2 2))}.

A final subtlety is that \textit{super-types} of \code{number} also contain
numerical data that must be extracted. Indeed, the type specifier \code{atom}
contains both numerical data---\code{(number * *)}---and non-numerical
