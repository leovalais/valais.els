...
\code{sb-kernel:specifier-type}, which also does type expansion. It also does
...
of the work of \code{subtypep}, as shown in \vref{fig:specifiertype}. %\vref{fig:stGTstp} and \vref{fig:stEQstp}.
...
keep another, more cost-effective solution.
...
result of an expansion may itself be an alias to expand\footnote{Fortunately,
  \code[\scriptsize]{sb-ext:typexpand} also returns a boolean
  indicating whether or not an expansion happened.}.
...
\code{(U * *)}\footnote{Implementations supporting the IEEE floating point
  raise many concerns with -0.0, NaN, $+\infty$ and $-\infty$. Baker explains
  in detail how to handle these cases, which our implementation does not support
  yet.}. Note the hereby new ``type specifier'' is likely \emph{not} to
be valid (e.g., \code{(fixnum * *)} is invalid).
...
\item Map each number, say $n$, to construct the type specifier
  \code{((type-of $n$) $n$ $n$)}\footnote{\code[\scriptsize]{type-of} sometimes
    returns a list which \code[\scriptsize]{car} is the desired type of the
    range.}.
...
% TODO perhaps give an rte expression of what the member type should
% look like
% \begin{itemize}
% \item \emph{A symbol} --- E.g., \code{fixnum}
% \item \emph{A range} --- E.g., \code{(integer 1 6)}
% \item \emph{A \code{member} expression} --- E.g., \code{(member 1 2 3)}
% \end{itemize}
