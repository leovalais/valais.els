...
efforts on), {\small\code{(subtypep 'boolean 'keyword)}} returns
...
Common Lisp types are not manipulated directly. Instead, the type to be
...
Common Lisp standard \cite{bib:ansi.94.cl}.
...
For example, \code{(or string symbol)} belongs to the literal type kingdom.
\code{(and number (not real))} belongs to the range type kingdom. However,
\code{(or symbol integer)} belongs to the literal type kingdom while
\code{(or symbol (integer * *))} belongs to both. This situation is handled in
section \ref{sec:pre}.
...
\begin{figure}
  \centering
  \newcommand\profgraph[1]{\includegraphics[scale=.42,trim={1mm 1mm 4mm 5mm},clip]{#1}}
...
different representations: a \emph{symbol} (e.g., \code{fixnum}), a
\emph{\code{member} expression} (e.g., \code{(member 1 2 3)}) or a \emph{range}
(e.g., \code{(integer 1 6)}). However, the first two belong to the literal type
kingdom whereas the latter belongs to the range kingdom.
Thus, the numerical type information would be distributed over the different
expert sub-procedures. For consistency and accuracy, a single internal
representation has to be chosen.
The symbolic and \code{member} numeric types must each be converted into an
equivalent type specifier, in which numerical data are only represented using
ranges.
...
\begin{proof}
  \emph{\small Type intersection, union and complement are equivalent to
    bitwise Boolean operations ``and'', ``or'' and ``not'' on
    representative bit-vectors.}
...
operations to fully discard the list representation.
...
\item Eq.~\ref{eq:anddist2} --- the distribution of the intersection over the union.
  Next step is Eq.~\ref{eq:andfin}.
\item Eq.~\ref{eq:anddist1} --- also the distribution of the intersection over the
...
Array type specifiers are complex to handle because they are bi-dimensional: it
...
array of \code{list} but rather an array of \code{(upgraded-array-element-type
  'list)}. For every value that might return this function, Baker
...
\begin{figure}
  ...
  \begin{tabular}{|cl|}
    ...
    \hline
  \end{tabular}
    \caption{Comparative efficiency measures of our \code{subtypep} implementation}
  \label{fig:perf}
\end{figure}
...
These results, shown in Figure~\ref{fig:perf}, are only presented here as
...
\begin{itemize}
\item Our implementation is overall slower in the range type kingdom.
\item Heavy users of \code{member} seems to experience a slower execution.
  Perhaps, as predicted by Baker, the reason is that the systematic registration
  of the elements makes the size of the bit-vectors grow quickly, thus making
  every subsequent operation slower.
\item For the symbolic type specifiers---primitive types, \textsc{Clos} classes
  and conditions---our implementation already outperforms \sbcl's.
\end{itemize}
