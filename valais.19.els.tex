...
\vref{lst:split} provides some examples of how the \code{split} function
behaves.

Our implementation uses a function \code{type-keep-if} which takes a predicate,
say $P$, and a type specifier that:
\begin{itemize}
\item given a type specifier $T$ such as $P(T) = \top$, returns it as it is,
\item given a type specifier $T$ such as $P(T) = \bot$, returns \code{nil},
\item and given a logic type specifier, runs recursively on each
  operand while keeping the operator (the symbol \code{or}, \code{and} or
  \code{not}).
\end{itemize}
...
\code{type-keep-if}, as shown in \vref{lst:split-impl}.
...
\end{clcode}
\caption{Implementation of \code{split}}
\label{lst:split-impl}
\end{listing}
