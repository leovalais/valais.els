...
\item \emph{The pre-processing} --- Both type specifiers are processed in order to
  ...
  also be split into equivalent sub-type specifiers restricted to each kingdom
  concerned.
...
\item As shown in \vref{eq:symbol}, the emptiness check passes, meaning that
  \code{symbol} is indeed a sub-type of \code{(member foo)}, which is obviously
  wrong.
  \begin{align}
...
The \code{keyword} sub-type of \code{symbol} is also subject to the problem.
