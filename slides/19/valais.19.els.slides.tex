...
\begin{frame}
  ...
  \item Baker's decision procedure
    \begin{enumerate}
      ...
      \it
    \item Pre-processing
    \item Types as bitsets
    \item Type specifier \rarr{} bitset expression
    \end{enumerate}
...
\end{clcode}
  ...
  \begin{popup}{.85}
    \onslide<2-3>
    \begin{macosbox}{Bitset expression reduction}
      \centering
...
\end{hlclcode}
    ...
    \item<2->[\alt<2>{\maybe}{\good}] \code{employee.half-time-p}
    \end{itemize}
...
\begin{frame}[fragile]
  ...
  \item Bitset $\bs_{\scriptsize\code{(member \plholder A \plholder B \plholder
        C)}}$
    \begin{enumerate}
