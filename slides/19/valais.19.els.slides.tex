...
\begin{frame}
  ...
  \begin{block}{Some\only<4->{ (unrealistic \emoji{🙈})} assumptions}
    \begin{itemize}
...
\end{smallclcode}
    ...
    \item<4-> Lookup bit-vectors \& translate logic operators
      % \begin{itemize}
      % \item with care to avoid misrepresentation \emoji{😛}
      % \end{itemize}
...
\begin{frame}[standout]
  ...
  \item \alt<2>{\sout{More efficient} \textit{April fools \emoji{😺}}}{More efficient}
  \end{itemize}
\end{frame}
